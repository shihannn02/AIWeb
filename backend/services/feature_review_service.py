...
SPECIAL_VALUES = getattr(_FF, "SPECIAL_VALUES", [-999, -1111])
SPECIAL_BIN_SENTINELS = getattr(_FF, "SPECIAL_BIN_SENTINELS", [-999, -1111])
_value_matches_special_bin = getattr(_FF, "_value_matches_special_bin")
...
_DETAIL_CACHE_VER = "v5"  # bump when value_type / detail shape changes


def _compute_iv_ks_from_bin_rows(bins: List[Dict[str, Any]]) -> Tuple[Optional[float], Optional[float]]:
    """由分箱 obs/bad 重算变量级 IV(total) 与 KS(total)。"""
    epsilon = 1e-6
    total_obs = sum(int(b.get("obs") or 0) for b in bins)
    total_bad = sum(int(b.get("bad") or 0) for b in bins)
    total_good = total_obs - total_bad
    if total_obs <= 0 or total_bad <= 0 or total_good <= 0:
        return None, None

    badattrs: List[float] = []
    goodattrs: List[float] = []
    for b in bins:
        obs = int(b.get("obs") or 0)
        bad = int(b.get("bad") or 0)
        if obs <= 0:
            continue
        badattrs.append(bad / total_bad)
        goodattrs.append((obs - bad) / total_good)

    if not badattrs:
        return None, None

    iv_bins = []
    for badattr, goodattr in zip(badattrs, goodattrs):
        woe = math.log((badattr + epsilon) / (goodattr + epsilon))
        iv_bins.append((badattr - goodattr) * woe)
    total_iv = float(sum(iv_bins))

    acu_bad = np.cumsum(badattrs)
    acu_good = np.cumsum(goodattrs)
    total_ks = float(np.max(np.abs(acu_bad - acu_good)))
    return round(total_iv, 6), round(total_ks, 4)


def _feature_binning_totals(bins: List[Dict[str, Any]]) -> Dict[str, Optional[float]]:
    """由当前展示的分箱行计算 IV(total) 与 KS(total)。"""
    total_iv, total_ks = _compute_iv_ks_from_bin_rows(bins)
    return {"total_iv": total_iv, "total_ks": total_ks}
...
def _categorical_value_matches_bin(value: Any, bin_label: str) -> bool:
    bdefs = [(bin_label, parse_bin_boundary(bin_label))]
    if assign_bin(value, bdefs) == bin_label:
        return True
...
def _assign_categorical_bin(value: Any, bin_order: List[str]) -> Optional[str]:
    ...
    assigned = assign_bin(value, bdefs)
...
def _parse_rule_from_bin(
    ...
    if rule_type == "head":
        if "(-inf" in lower:
            rhs = s.split(",", 1)[-1].strip().rstrip("]").strip()
    ...
    if lower.startswith("(-inf") or "(-inf" in lower:
        rhs = s.split(",", 1)[-1].strip().rstrip("]").strip()
        nums_hi = re.findall(r"-?\d+\.?\d*", rhs)
        if nums_hi:
            return "<=", float(nums_hi[0])
...
def _bins_for_feature(
    ...
    for _, r in sub.iterrows():
        ...
        rows.append({
            ...
            "is_high_bad": br > bad_rate_threshold and not _is_missing_bin_label(bl),
        })
    return rows
...
def _bins_for_feature_from_train_defs(
    ...
    numeric_like = sum(1 for bl in bin_order if _is_numeric_bin_label(bl))
    if value_type:
        use_cate = value_type == "categorical"
    else:
        use_cate = numeric_like <= len(bin_order) / 2
    if use_cate:
        ds["_bin"] = ds[feat].apply(lambda v: _assign_categorical_bin(v, bin_order))
    else:
        ds[feat] = pd.to_numeric(ds[feat], errors="coerce")
...
def _recalc_bin_feature_derived_columns(feat_df: pd.DataFrame) -> pd.DataFrame:
    """Test 重算 obs/bad 后，按箱顺序重算累计列与 IV/KS/WOE（避免沿用 Train 的 acu_*）。"""
    if feat_df.empty:
        return feat_df
    df = feat_df.copy()
    epsilon = 1e-6

    if "#Obs" in df.columns and "#Bad" in df.columns:
        obs = pd.to_numeric(df["#Obs"], errors="coerce").fillna(0)
        bad = pd.to_numeric(df["#Bad"], errors="coerce").fillna(0)
        total_all = float(obs.sum())
        bad_all = float(bad.sum())
        good_all = total_all - bad_all
        df["#Obs"] = obs.astype(int)
        df["#Bad"] = bad.astype(int)
        if "#Good" in df.columns:
            df["#Good"] = (obs - bad).astype(int)
        df["%Bad_Rate"] = np.where(obs > 0, bad / obs, 0.0)
        if total_all > 0 and bad_all > 0 and good_all > 0:
            badattr = bad / bad_all
            goodattr = (obs - bad) / good_all
            df["WOE"] = np.log((badattr + epsilon) / (goodattr + epsilon))
            if "IV(bin)" in df.columns:
                df["IV(bin)"] = (badattr - goodattr) * df["WOE"]
            if "IV(total)" in df.columns and "IV(bin)" in df.columns:
                df["IV(total)"] = df["IV(bin)"].cumsum()
            cum_bad_rate = bad_all / total_all
            if "Lift" in df.columns:
                df["Lift"] = np.where(cum_bad_rate > 0, df["%Bad_Rate"] / cum_bad_rate, 0.0)
        return df

    if "total" not in df.columns or "bad" not in df.columns:
        return df

    total = pd.to_numeric(df["total"], errors="coerce").fillna(0)
    bad = pd.to_numeric(df["bad"], errors="coerce").fillna(0)
    total_all = float(total.sum())
    bad_all = float(bad.sum())
    good_all = total_all - bad_all

    df["total"] = total.astype(int)
    df["bad"] = bad.astype(int)
    df["bad_rate"] = np.where(total > 0, bad / total, 0.0)

    if total_all <= 0:
        for c in (
            "badattr", "goodattr", "woe", "cum_bad_rate", "lift", "bins_iv", "total_iv",
            "acu_bin_badrate", "acu_bin_goodrate", "bin_ks", "total_ks",
            "acu_badnum", "acu_allnum", "acu_badrate",
        ):
            if c in df.columns:
                df[c] = 0.0
        return df

    df["badattr"] = bad / bad_all if bad_all > 0 else 0.0
    df["goodattr"] = (total - bad) / good_all if good_all > 0 else 0.0
    df["woe"] = np.log((df["badattr"] + epsilon) / (df["goodattr"] + epsilon))
    df["cum_bad_rate"] = bad_all / total_all
    df["lift"] = np.where(df["cum_bad_rate"] > 0, df["bad_rate"] / df["cum_bad_rate"], 0.0)
    df["bins_iv"] = (df["badattr"] - df["goodattr"]) * df["woe"]
    df["total_iv"] = df["bins_iv"].sum()
    df["acu_bin_badrate"] = df["badattr"].cumsum()
    df["acu_bin_goodrate"] = df["goodattr"].cumsum()
    df["bin_ks"] = (df["acu_bin_badrate"] - df["acu_bin_goodrate"]).abs().round(4)
    df["total_ks"] = df["bin_ks"].max()
    df["acu_badnum"] = df["bad"].cumsum()
    df["acu_allnum"] = df["total"].cumsum()
    df["acu_badrate"] = np.where(df["acu_allnum"] > 0, df["acu_badnum"] / df["acu_allnum"], 0.0)
    return df
...
def rebuild_test_binning_sheet(
    ...
    for feat in features:
        ...
        feat_out = pd.DataFrame(rebuilt)
        chunks.append(_recalc_bin_feature_derived_columns(feat_out))
...
def _feature_iv_bin_rows(feature_df: pd.DataFrame) -> List[Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    for _, r in feature_df.iterrows():
        tot = int(r.get("total", 0) or 0)
        if tot <= 0:
            continue
        bad = int(r.get("bad", 0) or 0)
        if bad <= 0 and r.get("bad_rate"):
            bad = int(round(tot * float(r.get("bad_rate", 0) or 0)))
        rows.append({
            "bin": str(r.get("bin_label", "")),
            "obs": tot,
            "bad": bad,
            "bad_rate": float(r.get("bad_rate", 0) or 0),
        })
    return rows


def _analyze_feature_iv(
    feat: str,
    train_norm: pd.DataFrame,
    train_total: int,
    iv_threshold: float,
    ...
    bin_rows = _feature_iv_bin_rows(ft)
    total_iv, total_ks = _compute_iv_ks_from_bin_rows(bin_rows)
    if total_iv is None or total_iv < iv_threshold:
        return None

    cn_name = get_chinese_name(feat)
    max_br = max((b["bad_rate"] for b in bin_rows), default=0.0)
    best = max(bin_rows, key=lambda b: (b["bad_rate"], b["obs"]))
    hit = int(best["obs"])

    if value_type == "categorical":
        high_bins = sorted(
            [{"bin": b["bin"], "obs": b["obs"], "bad_rate": b["bad_rate"]} for b in bin_rows],
            key=lambda x: (-x["bad_rate"], -x["obs"], x["bin"]),
        )
        return {
            "feature": feat,
            "chinese_name": cn_name,
            ...
            "iv_total": total_iv,
            "total_ks": total_ks,
            "max_bad_rate": float(max_br),
            "effect_label": "IV入选",
            "reason": f"IV(total) {total_iv:.4f} ≥ 阈值 {iv_threshold:.4f}，最高类别坏率 {max_br:.2%}",
            "rule_type": "iv_categorical",
            "hit_count": hit,
    ...
    if normal.empty:
        return None
    best_idx = int(normal["bad_rate"].idxmax())
    best_row = normal.loc[best_idx]
    rel_idx = int(normal.index.get_loc(best_idx))
    n = len(normal)
    op, val = _parse_rule_from_bin(str(best_row["bin_label"]), rel_idx, n)
    return {
        "feature": feat,
        "chinese_name": cn_name,
        "value_type": "numeric",
        ...
        "max_bad_rate": float(max_br),
        "effect_label": "IV入选",
        "reason": f"IV(total) {total_iv:.4f} ≥ 阈值 {iv_threshold:.4f}，最高箱坏率 {max_br:.2%}",
        "rule_type": "iv_numeric",
        "hit_count": int(best_row["total"]),
        "sample_pct": int(best_row["total"]) / train_total if train_total else 0.0,
        "category_key": classify_category(cn_name, feat),
        "rule_operator": op,
        "rule_threshold": val,
        "rule_display": f"{feat}{op}{_format_threshold(val)}",
        "rule_source_bin": str(best_row["bin_label"]),
        "rule_source_bad_rate": float(best_row["bad_rate"]),
        "rule_source_obs": int(best_row["total"]),
        "rule_meets_min_hit": True,
    }


def _iv_threshold_overview(
    train_norm: pd.DataFrame,
    train_total: int,
    ...
    for th in thresholds:
        ...
        for feat, ft in feature_groups.items():
            ...
            item = _analyze_feature_iv(feat, train_norm, train_total, th, value_type=vtype)
            if item:
                count += 1
        overview.append({
            "threshold": th,
            "threshold_pct": f"{th:.2f}",
            "feature_count": count,
            "avg_hit": 0,
            "hint": f"约 {count} 个变量 IV(total) ≥ {th:.2f}（基于等频分箱结果）",
        })
    return overview


def _build_report_text_iv(
    train_br: float,
    ...
    lines = [
        f"Train 大盘坏率 {train_br:.2%}，当前 IV 阈值 {iv_threshold:.4f}。",
        f"在该阈值下共筛出 {auto_n} 个 IV 达标变量",
    ]
    if manual_added:
        lines[-1] += f"，另手动追加 {manual_added} 个"
    lines[-1] += "。请在下一步查看等频分箱明细与逾期率，并调整拒绝阈值。"
    if test_br is not None:
        lines.insert(1, f"Test 大盘坏率 {test_br:.2%}。")
    lines.append("建议：先看「IV 阈值概览」表格，再进入单变量挑选逐条确认分箱。")
    return " ".join(lines)
...
def _apply_month_usable(frame: pd.DataFrame) -> bool:
    if frame.empty or "apply_month" not in frame.columns:
        return False
    s = frame["apply_month"].astype(str).str.strip().str.lower()
    ok = frame["apply_month"].notna() & ~s.isin(("", "nan", "none", "nat", "<na>"))
    return bool(ok.any())


def _inject_apply_month(frame: pd.DataFrame, time_col: str) -> None:
    """从第 1 步选择的时间列（或 apply_date 等）生成 apply_month。"""
    if frame.empty:
        return
    if _apply_month_usable(frame):
        return
    month_src = None
    for c in (time_col, "apply_date", "apply_time", "create_time_x"):
    ...
    if pd.api.types.is_numeric_dtype(src):
        ...
        if dt.notna().mean() < 0.5:
            dt = pd.to_datetime(src, unit="ms", errors="coerce")
    else:
...
def _prepare_stability_frames(
    ...
    for frame in (tr, te):
        _inject_apply_month(frame, time_col)
...
def _stability_table(
    ...
    if (value_type or _column_value_type(data, feature)) == "categorical":
        ...
        ds[feature] = pd.to_numeric(ds[feature], errors="coerce")
...
def build_threshold_overview(
    ...
    mode = screening_mode if screening_mode in ("bad_rate", "iv_equal_freq") else "bad_rate"
    if mode == "iv_equal_freq":
        overview = _iv_threshold_overview(train, train_total, raw_df=raw_df)
    else:
        overview = _threshold_overview(
            train, train_total, train_overall, min_hit_count, raw_df=raw_df
        )
    return {
        "screening_mode": mode,
        "threshold_overview": overview,
    }
...
def _review_result_cache_key(
    ...
    return (
        f"{job_id}|{screening_mode}|{bad_rate_threshold:.4f}|{iv_threshold:.4f}|"
        f"{min_hit_count}|{inc}|{int(include_threshold_overview)}"
    )
...
def build_feature_review(
    ...
    cache_key = _review_result_cache_key(
        ...
        include_features, include_threshold_overview, mode, iv_threshold,
    )
    ...
    for feat in all_feature_names:
        ...
        if mode == "iv_equal_freq":
            item = _analyze_feature_iv(
                feat, train, train_total, iv_threshold, value_type=vtype,
            )
        else:
        ...
        if mode == "iv_equal_freq" and item:
            light_item["iv_total"] = item.get("iv_total")
        all_features_light.append(light_item)
        if item:
            results.append(item)
            seen.add(feat)
    ...
    if mode == "iv_equal_freq":
        results.sort(key=lambda r: (-float(r.get("iv_total") or 0), -r["max_bad_rate"]))
    else:
    ...
    result = {
        ...
        "threshold_overview": (
            _iv_threshold_overview(train, train_total, raw_df=raw_df)
            if mode == "iv_equal_freq" and include_threshold_overview
            else _threshold_overview(train, train_total, train_overall, min_hit_count, raw_df=raw_df)
            if include_threshold_overview
            else []
        ),
        "report_summary": (
            _build_report_text_iv(
                train_overall, test_overall, iv_threshold, len(results), manual_added,
            )
            if mode == "iv_equal_freq"
            else _build_report_text(
...
def get_feature_detail(
    ...
    if _bins_need_train_recalc(train_bins):
        try:
            ...
            if train_bins_recalc and sum(int(b.get("obs") or 0) for b in train_bins_recalc) > 0:
                flags_by_bin = {str(b["bin"]): b for b in train_bins}
                for row in train_bins_recalc:
                    prev = flags_by_bin.get(str(row["bin"]))
                    if prev:
                        row["is_rule_bin"] = prev.get("is_rule_bin", False)
                        row["is_high_bad"] = prev.get("is_high_bad", False)
    ...
    detail: Dict[str, Any] = {
        ...
        "binning_totals": {
            "train": _feature_binning_totals(train_bins),
            "test": _feature_binning_totals(test_bins),
        },
...
def _portfolio_stats(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if "money" in df.columns:
        money = float(pd.to_numeric(df["money"], errors="coerce").fillna(0).sum())
    if not math.isfinite(money):
        money = 0.0
    bad_money = 0.0
    if bad and "money" in df.columns:
        bad_money = float(
            pd.to_numeric(df.loc[df["overdue_flag"] == 1, "money"], errors="coerce").fillna(0).sum()
        )
        if not math.isfinite(bad_money):
            bad_money = 0.0
    return {
        "count": n,
        "bad_rate": bad / n,
...
def _rule_hit_mask(
    ...
    *,
    ...
    bad_rate_threshold: float = 0.60,
    min_hit: int = 10,
    force_raw_numeric: bool = False,
) -> pd.Series:
    if feature not in df.columns:
        return pd.Series(False, index=df.index)
    op = operator.strip()
    if op == "in":
        if not values and not source_bins:
            return pd.Series(False, index=df.index)
        selected = [str(v) for v in (values or source_bins or [])]
    ...
    if force_raw_numeric:
        s = pd.to_numeric(df[feature], errors="coerce")
        for sv in SPECIAL_BIN_SENTINELS:
            s = s.mask(pd.to_numeric(df[feature], errors="coerce") == sv, np.nan)
        th = float(threshold)
        if op == ">":
            return s > th
        if op == ">=":
    ...
    if train_detail_df is not None:
        ...
        if bin_order:
            ...
            target_bins = _numeric_rule_target_bins(bdefs, op, threshold)
            if target_bins:
                return assigned.astype(str).isin({str(b) for b in target_bins})

    # 无分箱边界时兜底：裸数值比较（不含特殊值箱）
    s = pd.to_numeric(df[feature], errors="coerce")
    for sv in SPECIAL_BIN_SENTINELS:
        s = s.mask(pd.to_numeric(df[feature], errors="coerce") == sv, np.nan)
    th = float(threshold)
