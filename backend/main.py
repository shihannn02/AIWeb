...
from services.multivariate_service import (
    evaluate_mixed_reject_preview,
    filter_multivariate_rules,
    get_compound_rule_stability,
    mine_multivariate_rules,
)
from services.iv_pair_grid_service import (
    build_pair_grid_detail,
    build_pair_grids_overview,
)
...
async def upload_file(
    ...
    label, info = await loop.run_in_executor(None, partial(_parse_uploaded_file, path))
    return {
        "file_id": file_id,
        ...
        "suggested_time_col": guess_best_time_col(info["column_names"]),
        "default_drop_cols": [c for c in DEFAULT_DROP_COLS if c in info["column_names"]],
...
class RejectRuleItem(BaseModel):
    ...
    rule_display: Optional[str] = None
    conditions: Optional[List[RuleConditionItem]] = None
    features: Optional[List[str]] = None
    source_bins: Optional[List[str]] = None


class MultivariateMineRequest(BaseModel):
    features: List[str] = []
    comb: int = 3
    tree_depth: int = 3
    max_leaf_nodes: int = 4
    min_samples_leaf_frac: float = 0.005
    lift_threshold: float = 1.0
    combo_random: float = 0.1
    sample_multiple: int = 3
    need_rm_narow: bool = False
    skip_feature_filter: bool = True


class MultivariateFilterRequest(BaseModel):
    bad_rate_threshold: float = 0.60
    min_hit_count: int = 10
    features: List[str] = []
    comb: int = 3
    tree_depth: int = 3
    combo_random: float = 0.3


class IvPairGridsRequest(BaseModel):
    features: List[str] = []


class IvPairGridDetailRequest(BaseModel):
    feature_a: str
    feature_b: str
    # 仅内部切分点（不含 ±inf），如 [0, 1, 5, 11]
    bin_edges_a: Optional[List[float]] = None
    bin_edges_b: Optional[List[float]] = None


class MixedRejectPreviewRequest(BaseModel):
    compound_rules: List[RejectRuleItem] = []
    single_rules: List[RejectRuleItem] = []


class CompoundStabilityRequest(BaseModel):
    conditions: List[RuleConditionItem] = []
...
def feature_review_get(
    ...
    bad_rate_threshold: float = 0.60,
    min_hit_count: int = 10,
    include_features: Optional[str] = None,
    ...
    try:
        extra = [f.strip() for f in (include_features or "").split(",") if f.strip()]
        return build_feature_review(
            job_id,
            bad_rate_threshold=bad_rate_threshold,
            min_hit_count=min_hit_count,
            include_features=extra or None,
            include_threshold_overview=include_threshold_overview,
...
def feature_review_post(
    ...
    try:
        return build_feature_review(
            ...
            screening_mode=body.screening_mode,
            iv_threshold=body.iv_threshold,
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))


@app.post("/api/binning/{job_id}/warmup")
def review_warmup(job_id: str, _: str = Depends(get_current_user)):
    try:
        return warmup_review_cache(job_id)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))


@app.get("/api/binning/{job_id}/threshold-overview")
def threshold_overview(
    ...
    try:
        return build_threshold_overview(job_id, min_hit_count=min_hit_count, screening_mode=screening_mode)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))


@app.post("/api/binning/{job_id}/multivariate/mine")
def multivariate_mine(
    job_id: str,
    body: MultivariateMineRequest,
    _: str = Depends(get_current_user),
):
    try:
        params = {
            ...
            "max_leaf_nodes": body.max_leaf_nodes,
            "min_samples_leaf_frac": body.min_samples_leaf_frac,
            "lift_threshold": body.lift_threshold,
            "combo_random": body.combo_random,
            "sample_multiple": body.sample_multiple,
            "need_rm_narow": body.need_rm_narow,
            "skip_feature_filter": body.skip_feature_filter,
        }
        return mine_multivariate_rules(job_id, body.features, params)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"多变量挖掘失败: {e}")


@app.post("/api/binning/{job_id}/multivariate/filter")
def multivariate_filter(
    job_id: str,
    body: MultivariateFilterRequest,
    _: str = Depends(get_current_user),
):
    try:
        mined = mine_multivariate_rules(
            job_id,
            body.features,
            {
                "comb": body.comb,
        ...
        filtered = filter_multivariate_rules(
            mined, body.bad_rate_threshold, body.min_hit_count
        )
        return {
            **mined,
        ...
        raise HTTPException(status_code=500, detail=f"多变量筛选失败: {e}")


@app.post("/api/binning/{job_id}/iv-pair-grids")
def iv_pair_grids_overview(
    job_id: str,
    body: IvPairGridsRequest,
    _: str = Depends(get_current_user),
):
    try:
        return build_pair_grids_overview(job_id, body.features)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"IV 组合格子生成失败: {e}")


@app.post("/api/binning/{job_id}/iv-pair-grid/detail")
def iv_pair_grid_detail(
    job_id: str,
    body: IvPairGridDetailRequest,
    _: str = Depends(get_current_user),
):
    try:
        return build_pair_grid_detail(
            job_id,
            body.feature_a,
            body.feature_b,
            bin_edges_a=body.bin_edges_a,
            bin_edges_b=body.bin_edges_b,
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"IV 组合格子详情失败: {e}")


@app.post("/api/binning/{job_id}/multivariate/reject-preview")
def multivariate_reject_preview(
    job_id: str,
    body: MixedRejectPreviewRequest,
    _: str = Depends(get_current_user),
):
    try:
        compound = [r.model_dump() for r in body.compound_rules]
        single = [r.model_dump() for r in body.single_rules]
        return evaluate_mixed_reject_preview(job_id, compound, single)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"多变量拒绝预览失败: {e}")


@app.post("/api/binning/{job_id}/multivariate/stability")
def multivariate_stability(
    job_id: str,
    body: CompoundStabilityRequest,
    _: str = Depends(get_current_user),
):
    try:
        conditions = [c.model_dump() for c in body.conditions]
        return get_compound_rule_stability(job_id, conditions)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"组合规则月度稳定性失败: {e}")
