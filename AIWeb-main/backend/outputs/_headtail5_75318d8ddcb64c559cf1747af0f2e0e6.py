...
file_path = r'E:\AIWeb-main\AIWeb-main\backend\uploads\789033dd2029433481a825857f0e0ef4.csv'
output_file = r'E:\AIWeb-main\AIWeb-main\backend\outputs\分箱结果_headtail5_d406df05.xlsx'
label = 'target'
time_col = 'loan_date'
...
drop_cols = ['client_id', 'apply_id', 'money', 'fact_money', 'fact_repay_money', 'risk_over_days', 'pk_id', 'source_type', 'loan_term_bill_pid', 'target']
...
def _binning_worker_count(feature_count: int) -> int:
    ...
    # Windows spawn 开销大：默认串行，仅当显式设置 BINNING_WORKERS>1 才并行
    if os.name == "nt":
        if not env:
            return 1
        try:
...
def _parallel_map(func, tasks, init_fn=None, init_args=()):
    ...
    if os.name == "nt" or workers <= 1 or len(tasks) <= 1:
...
def _ht5_main():
    ...
    for sample_type_sub in sample_type:
        ...
        for target_sub in target:
            ...
            for var in mydata1.columns[:-1]:
                ...
                var_select_rows.append([
                    'wtt'+str(seq), datetime.now().strftime('%Y-%m-%d'), sample_type_sub, target_sub,
    ...
    for sample_type_sub in sample_type:
        ...
        for target_sub in target:
            ...
            for var, edge, merge in _parallel_map(_ht5_fit_one_var, fit_tasks):
                if edge is not None:
    ...
    if actual_oot:
        ...
        for sample_type_sub in sample_type:
            ...
            for target_sub in target:
                ...
                test_processed = len(apply_tasks)
                for merge in _parallel_map(_ht5_apply_one_var, apply_tasks):
                    if merge is not None:
