...
OTHER_MISSING_VALUES = [
    -9999, -999999, -99999, -9999979, -999979, -999990,
    -9999979.0, -999979.0, -999990, -9999.0,
    "-9999979", "-999979", "-999999", "-99999", "-99",
    "-9999.0", "-9999", "-9999979.0", "-99999.0",
    "", " ", "NaN", "None", "none", "NaT",
]
SPECIAL_VALUES = SPECIAL_BIN_SENTINELS + OTHER_MISSING_VALUES


def _parse_special_bin_key(bin_label):
    s = str(bin_label).strip()
    lower = s.lower()
    if lower.startswith("special(") and s.endswith(")"):
        inner = s[8:-1].strip().lower()
        if inner in ("nan", "none", "null", "缺失"):
            return "nan"
        try:
            v = float(inner)
            ...
            return inner
    if lower in ("missing", "缺失值") or any(k in s for k in ("缺失", "MISSING")):
        return "nan"
    return None


def _value_matches_special_bin(value, special_key):
    if special_key == "nan":
        if value is None or (isinstance(value, float) and math.isnan(value)):
            return True
        coerced = _coerce_bin_numeric(value)
        return coerced is None or value in OTHER_MISSING_VALUES
    coerced = _coerce_bin_numeric(value)
    if coerced is None:
        return False
    try:
        target = float(special_key)
        return abs(coerced - target) < 1e-9
    except (TypeError, ValueError):
        return str(value).strip() == str(special_key)


def parse_bin_boundary(bin_label):
    """Return (lo, hi, left_closed, right_closed, is_missing[, special_key])"""
    s = str(bin_label).strip()
    lower = s.lower()
    sp_key = _parse_special_bin_key(s)
    if sp_key is not None:
        return (None, None, False, False, True, sp_key)
    if lower == 'missing':
        return (None, None, False, False, True, 'nan')
    if any(k in s for k in ['特殊','空值','nan','NA','null']) and 'special(' not in lower:
        return (None, None, False, False, True, 'nan')
...
def _coerce_bin_numeric(value):
    """分箱赋值前尽量转为 float；无法转换则原样返回。"""
    if value is None:
        return None
...
def assign_bin(value, bin_defs):
    """Assign value to bin; special(nan)/special(-999)/special(-1111) 各自匹配。"""
    for bl, meta in bin_defs:
        if meta is None:
            continue
        if len(meta) >= 6 and meta[4] is True:
            if _value_matches_special_bin(value, meta[5]):
                return bl

    is_nan = value is None or (isinstance(value, float) and math.isnan(value))
    coerced = _coerce_bin_numeric(value)
    if is_nan or coerced is None or value in OTHER_MISSING_VALUES:
        for bl, meta in bin_defs:
            if meta is None:
                continue
            if len(meta) >= 5 and meta[4] is True:
                sk = meta[5] if len(meta) >= 6 else None
                if sk in (None, 'nan'):
                    return bl
        return None
    if coerced is value and not isinstance(coerced, (int, float)):
        return None
    value = coerced
