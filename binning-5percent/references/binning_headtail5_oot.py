...
SPECIAL_BIN_SENTINELS = [-999, -1111]


def _is_special_bin_label(label):
    s = str(label)
    return s.startswith("special(") or s == "缺失值"


def _data_for_numeric_binning(data, factor_name):
    mask = data[factor_name].notnull()
    for sv in SPECIAL_BIN_SENTINELS:
        mask &= data[factor_name] != sv
    return data[mask]


def _special_bin_mask(series, bin_label):
    s = str(bin_label)
    if s == "special(nan)" or s == "缺失值":
        return series.isnull()
    if s == "special(-999)":
        return series == -999
    if s == "special(-1111)":
        return series == -1111
    if s.startswith("special(") and s.endswith(")"):
        inner = s[8:-1].strip()
        try:
            v = float(inner)
            return series == v
...
# Web 注入：manual 模式下的 test 文件路径（None 表示非 manual）
TEST_FILE_PATH = None
...
# ProcessPool 子进程共享状态（避免每个变量重复 pickle y / 元数据）
_HT5_MP_STATE: dict = {}
...
def _binning_worker_count(feature_count: int) -> int:
    ...
    win_parallel_min = int(os.getenv("BINNING_WIN_PARALLEL_MIN_VARS", "1200") or "1200")
    # Windows spawn 开销大：中小任务默认串行；变量极多时再开有限并行
    if os.name == "nt":
        if not env:
            if feature_count >= win_parallel_min:
                cpus = os.cpu_count() or 4
                return min(max(cpus - 1, 2), 4, feature_count)
            return 1
        try:
            n = int(env)
        except ValueError:
...
def _use_parallel(task_count: int) -> bool:
    """变量足够多时用优化后的 ProcessPool；过少或显式单线程时走串行。"""
    if task_count <= 1:
        return False
    if os.getenv("BINNING_FORCE_PARALLEL", "").strip() == "1":
        return True
    if os.getenv("BINNING_DISABLE_PARALLEL", "").strip() == "1":
        return False
    if os.getenv("BINNING_WORKERS", "").strip() == "1":
        return False
    min_tasks = int(os.getenv("BINNING_PARALLEL_MIN_TASKS", "30") or "30")
    return task_count >= min_tasks


def _build_var_msg_lookup(var_select_df: pd.DataFrame) -> dict:
    lookup = {}
    if var_select_df.empty:
        return lookup
    for key, grp in var_select_df.groupby(["样本类型", "变量英文名", "坏客户定义"], sort=False):
        lookup[key] = grp[_VAR_MSG_COLS].to_dict("records")
    return lookup


def _ht5_mp_init_fit(
    sample_type_sub, target_sub, min_rate, sub_div_bin, min_num,
    y_vals, var_msg_lookup, cate_cols,
):
    global _HT5_MP_STATE
    _HT5_MP_STATE = {
        "mode": "fit",
        "sample_type_sub": sample_type_sub,
        "target_sub": target_sub,
        "min_rate": min_rate,
        "sub_div_bin": sub_div_bin,
        "min_num": min_num,
        "y_vals": y_vals,
        "var_msg_lookup": var_msg_lookup,
        "cate_cols": set(cate_cols),
    }


def _ht5_mp_init_apply(sample_type_sub, target_sub, y_vals, var_msg_lookup, cate_cols):
    global _HT5_MP_STATE
    _HT5_MP_STATE = {
        "mode": "apply",
        "sample_type_sub": sample_type_sub,
...
def _parallel_map(func, tasks, init_fn=None, init_args=()):
    ...
    if init_fn:
        pool_kwargs["initializer"] = init_fn
        pool_kwargs["initargs"] = init_args
    with ProcessPoolExecutor(**pool_kwargs) as executor:
        return list(executor.map(func, tasks, chunksize=chunksize))


def _append_var_select_rows(
    rows, seq, sample_type_sub, target_sub, sample_range, mydata1, target_col,
):
    """逐列统计元数据（避免每列 copy 子 DataFrame）。"""
    target_arr = mydata1[target_col].values
    total_len = len(target_arr)
    total_bad = int(target_arr.sum())
    bad_rate_withna = float(target_arr.mean()) if total_len else 0.0
    dt_str = datetime.now().strftime("%Y-%m-%d")
    nan_pad = [np.nan] * 25
    for var in mydata1.columns[:-1]:
        s = mydata1[var]
        na_count = int(s.isnull().sum())
        na_rate = na_count / total_len if total_len else 0.0
        unique_withna = s.nunique(dropna=False)
        if na_count < total_len:
            nona_mask = ~s.isnull().values
            bad_rate_nona = float(target_arr[nona_mask].mean())
        else:
            bad_rate_nona = 0.0
        rows.append([
            "wtt" + str(seq), dt_str, sample_type_sub, target_sub,
            var, var, sample_range, bad_rate_nona, bad_rate_withna, total_len,
            total_bad, na_count, na_rate, unique_withna,
        ] + nan_pad + ["Y"])
        seq += 1
    return seq


def _ht5_fit_one_var_core(
    var, sample_type_sub, target_sub, min_rate, sub_div_bin, min_num,
    is_cate, x_vals, y_vals, var_msg_records,
):
    mydata1 = pd.DataFrame({var: x_vals, target_sub: y_vals})
    try:
        if is_cate:
        ...
        sample_bin["WOE(total)"] = (
            (sample_bin["WOE"] * sample_bin["#Obs"]).sum() / total_obs if total_obs > 0 else np.nan
        )
        sample_bin["变量英文名"] = var
        sample_bin["样本类型"] = sample_type_sub
        sample_bin["坏客户定义"] = target_sub
        if not var_msg_records:
            return var, edge, None
        var_msg = pd.DataFrame(var_msg_records)
        merge = pd.merge(
            var_msg, sample_bin,
            on=["变量英文名", "样本类型", "坏客户定义"], how="left",
        )
        return var, edge, merge
    except Exception:
        return var, None, None


def _ht5_apply_one_var_core(
    var, sample_type_sub, target_sub, is_cate, x_vals, y_vals,
    train_info, var_msg_records,
):
    if not train_info or not var_msg_records:
        return None
    mydata1_test = pd.DataFrame({var: x_vals, target_sub: y_vals})
    try:
        if train_info.get("type") == "cate":
            ...
            k_test = group_by_var_value(
                _data_for_numeric_binning(mydata1_test, var), target_sub, var, "#Bad", "#Good"
            )
            k1_test = get_special_bins(mydata1_test, target_sub, var, "#Bad", "#Good")
            if len(k_test) == 0:
                return None
        ...
        ks_values = calculate_ks(good, bad)
        sample_bin_test = sample_bin_test.copy()
        ...
        sample_bin_test["WOE(total)"] = (
            (sample_bin_test["WOE"] * sample_bin_test["#Obs"]).sum() / total_obs if total_obs > 0 else np.nan
        )
...
def _ht5_fit_one_var(task):
    return _ht5_fit_one_var_core(*task)


def _ht5_fit_one_var_mp(task):
    var, x_vals = task
    st = _HT5_MP_STATE
    key = (st["sample_type_sub"], var, st["target_sub"])
    var_msg_records = st["var_msg_lookup"].get(key, [])
    return _ht5_fit_one_var_core(
        var, st["sample_type_sub"], st["target_sub"],
        st["min_rate"], st["sub_div_bin"], st["min_num"],
        var in st["cate_cols"], x_vals, st["y_vals"], var_msg_records,
    )


def _ht5_apply_one_var(task):
    return _ht5_apply_one_var_core(*task)


def _ht5_apply_one_var_mp(task):
    var, x_vals, train_info = task
    st = _HT5_MP_STATE
    key = (st["sample_type_sub"], var, st["target_sub"])
    var_msg_records = st["var_msg_lookup"].get(key, [])
    return _ht5_apply_one_var_core(
        var, st["sample_type_sub"], st["target_sub"],
        var in st["cate_cols"], x_vals, st["y_vals"], train_info, var_msg_records,
    )
...
def get_special_bins(data_total, flag_name, factor_name, good_name, bad_name):
    """nan / -999 / -1111 各单独一箱（有样本才输出）。"""
    rows = []
    na_data = data_total[data_total[factor_name].isnull()]
    if len(na_data) > 0:
        good_cnt = na_data[flag_name].sum()
        tn = len(na_data[na_data[flag_name].notnull()])
        rows.append(["special(nan)", good_cnt, tn - good_cnt])
    for sv in SPECIAL_BIN_SENTINELS:
        sv_data = data_total[data_total[factor_name] == sv]
        if len(sv_data) > 0:
            good_cnt = sv_data[flag_name].sum()
            tn = len(sv_data[sv_data[flag_name].notnull()])
            rows.append([f"special({sv})", good_cnt, tn - good_cnt])
    if not rows:
        return pd.DataFrame(columns=[factor_name, good_name, bad_name])
    return pd.DataFrame(rows, columns=[factor_name, good_name, bad_name])


def get_na_bin(data_total, flag_name, factor_name, good_name, bad_name):
    return get_special_bins(data_total, flag_name, factor_name, good_name, bad_name)
...
def important_bin_calculate(data_df,na_df, good_name, bad_name, factor_name, knots_list,if_sort=False):
    ...
    if len(na_df) != 0:
        ...
        for _, srow in na_df.iterrows():
            temp_df_list.append(pd.DataFrame([srow]))
            bin_list.append(srow[factor_name])
    else:
        na_good = 0
        na_bad = 0
    ...
    if len(na_df) != 0:
        special_bins = set(na_df[factor_name].astype(str))
        n_special = len(na_df)
        result_indicator_1 = result_indicator[~result_indicator['Bin'].isin(special_bins)]
        result_indicator['Odds1']=((result_indicator_1['#Cum_Bad']/result_indicator_1['#Cum_Good'])/(
            (total_bad -na_bad- result_indicator_1['#Cum_Bad'])/(total_good -na_good- result_indicator_1['#Cum_Good']))).tolist()+[np.nan]*n_special
        odds2_body = list(1/pd.Series(result_indicator['Odds1']).replace(0, np.nan))[0:-(n_special+1)]
        result_indicator['Odds2']=[np.nan]+odds2_body+[np.nan]*n_special
    else:
...
def get_bin_lift_with_edges(data, flag_name, factor_name, min_rate=0.001, sub_div_bin=0.1, min_num=5, method='best', max_bins=50, numOfSplit=30):
    ...
    numeric_data = _data_for_numeric_binning(data, factor_name)
    k = group_by_var_value(numeric_data, flag_name, factor_name, '#Bad', '#Good')
    k1 = get_special_bins(data, flag_name, factor_name, '#Bad', '#Good')
    if len(k) == 0:
...
def transform_cate_series(series, valid_cats=None):
    ...
    s = s.replace(['-9999', '-999999', 'nan', 'None', '', 'NaT'], np.nan)
    s = s.fillna('MISSING')
...
def _coerce_time_series(series):
    """与 Web 端 data_service 一致：兼容 ISO、斜杠日期、Unix 秒/毫秒时间戳等。"""
...
def _ht5_main():
    ...
    if TEST_FILE_PATH:
        ...
        if str(test_file_path).endswith('.csv'):
            test_data = None
            for _enc in ('utf-8', 'gbk', 'gb18030', 'latin1'):
                try:
        ...
        if use_cutoff:
            _times = _coerce_time_series(data[time_col])
            ...
            data = data.assign(__sort_time=_times).sort_values('__sort_time')
            _times_sorted = data['__sort_time']
            oot_date = _times_sorted.quantile(1 - OOT_RATIO)
            train_data = data[_times_sorted < oot_date].drop(columns='__sort_time').copy()
            test_data = data[_times_sorted >= oot_date].drop(columns='__sort_time').copy()
            print(f'Train: {len(train_data)}, Test: {len(test_data)}')
            if len(train_data) == 0 or len(test_data) == 0:
    ...
    # ==================== CORE BINNING FUNCTIONS ====================
    # 数值分箱使用模块级 group_by_var_value / get_special_bins / important_bin_calculate / get_bin_lift_with_edges
    ...
    for sample_type_sub in sample_type:
        ...
        for target_sub in target:
            ...
            seq = _append_var_select_rows(
                var_select_rows, seq, sample_type_sub, target_sub, sample_range, mydata1, target_sub,
            )

    var_select_01 = pd.DataFrame(var_select_rows, columns=col_seq) if var_select_rows else pd.DataFrame(columns=col_seq)
    var_select_01['标签2'] = 'N'
    var_select_01['标签3'] = 'N'
    var_msg_lookup = _build_var_msg_lookup(var_select_01)
    ...
    for sample_type_sub in sample_type:
        ...
        for target_sub in target:
            ...
            y_vals = mydata1[target_sub].values
            feature_cols = list(mydata1.columns[:-1])
            if _use_parallel(len(feature_cols)):
                mp_tasks = [(var, mydata1[var].values) for var in feature_cols]
                fit_iter = _parallel_map(
                    _ht5_fit_one_var_mp,
                    mp_tasks,
                    init_fn=_ht5_mp_init_fit,
                    init_args=(
                        sample_type_sub, target_sub, min_rate, sub_div_bin, min_num,
                        y_vals, var_msg_lookup, cate_cols,
                    ),
                ...
                for var in feature_cols:
                    key = (sample_type_sub, var, target_sub)
                    var_msg_records = var_msg_lookup.get(key, [])
                    fit_tasks.append((
                        var, sample_type_sub, target_sub, min_rate, sub_div_bin, min_num,
                        var in cate_cols, mydata1[var].values, y_vals, var_msg_records,
                    ))
                fit_iter = _parallel_map(_ht5_fit_one_var, fit_tasks)
            for var, edge, merge in fit_iter:
                if edge is not None:
                    train_stored_edges[var] = edge
    ...
    if len(train_bins_detail) > 0 and money_train is not None:
        ...
        for _, row in train_bins_detail.iterrows():
            ...
            try:
                sp_mask = _special_bin_mask(bindata_with_money[var], bin_label)
    ...
    if actual_oot:
        ...
        for sample_type_sub in sample_type:
            ...
            for target_sub in target:
                ...
                y_vals_test = mydata1_test[target_sub].values
                apply_vars = [
                    var for var in mydata1_test.columns[:-1]
                    if var in train_stored_edges
                ]
                test_processed = len(apply_vars)
                if _use_parallel(len(apply_vars)):
                    mp_apply_tasks = [
                        (var, mydata1_test[var].values, train_stored_edges[var])
                        for var in apply_vars
                    ]
                    apply_iter = _parallel_map(
                        _ht5_apply_one_var_mp,
                        mp_apply_tasks,
                        init_fn=_ht5_mp_init_apply,
                        init_args=(
                            sample_type_sub, target_sub, y_vals_test, var_msg_lookup, cate_cols,
                        ),
                    )
                else:
                    apply_tasks = []
                    for var in apply_vars:
                        ...
                        var_msg_records = var_msg_lookup.get(key, [])
                        apply_tasks.append((
                            var, sample_type_sub, target_sub, var in cate_cols,
                            mydata1_test[var].values, y_vals_test,
                            train_stored_edges[var], var_msg_records,
                        ))
                    apply_iter = _parallel_map(_ht5_apply_one_var, apply_tasks)
                for merge in apply_iter:
                    if merge is not None:
        ...
        if len(test_bins_detail) > 0 and money_test is not None:
            ...
            for _, row in test_bins_detail.iterrows():
                ...
                try:
                    sp_mask = _special_bin_mask(bindata_with_money_test[var], bin_label)
                    if sp_mask is not None:
                        mask = sp_mask
                    elif bin_label.startswith('(-inf,'):
