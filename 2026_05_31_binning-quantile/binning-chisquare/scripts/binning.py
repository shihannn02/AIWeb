...
OTHER_MISSING_VALUES = [
    ...
    "", " ", "NaN", "None", "none", "NaT", pd.NaT,
]


def _column_missing_rate(series):
    s = pd.Series(series)
    mask = s.isna() | s.isin(OTHER_MISSING_VALUES)
    for sv in SPECIAL_BIN_SENTINELS:
        mask = mask | (s == sv)
    return float(mask.mean()) if len(s) else 0.0


def _split_normal_special(x, y):
    x = pd.Series(x)
    y = pd.Series(y)
    nan_mask = x.isna() | x.isin(OTHER_MISSING_VALUES)
    sv_mask = (~nan_mask) & x.isin(SPECIAL_BIN_SENTINELS)
    special_mask = nan_mask | sv_mask
    return x[~special_mask], y[~special_mask]


def _build_special_bin_rows(x, y):
    x = pd.Series(x)
    y = pd.Series(y)
    rows = []
    nan_mask = x.isna() | x.isin(OTHER_MISSING_VALUES)
...
def transform_preprocess(df, large_cat_info):
    ...
    df[num_cols] = df[num_cols].replace(OTHER_MISSING_VALUES, np.nan)
...
def analyze_missing_distribution(train, step=0.05):
    """分析缺失率分布，帮助决定阈值"""
    missing_rate = train.apply(_column_missing_rate)
...
def fit_chisquare_bins(x, y, bin_num=10, init_bin_num=20, special_values=None):
    # Learn chi-square bin edges from training data
    epsilon = 1e-6
    x_normal, y_normal = _split_normal_special(x, y)
    ...
    if len(x_normal) > 1:
        ...
        bin_edges = [-np.inf, np.inf]
    return bin_edges, SPECIAL_BIN_SENTINELS


def apply_bin_frequency(x, y, bin_edges=None, special_values=None):
    ...
    # Special value bins: nan / -999 / -1111
    special_rows = _build_special_bin_rows(x, y)
    if special_rows:
        d_special = pd.DataFrame(special_rows)
        d3 = pd.concat([d3, d_special], ignore_index=True) if len(d3) else d_special
